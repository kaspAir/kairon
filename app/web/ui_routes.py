...
def _safe_decision_status(decision: Decision) -> str:
    return normalize_decision_status(getattr(decision, "status", None))


def _governance_state(decision: Decision) -> str:
    decision_status = _safe_decision_status(decision)
    if decision_status in {"observed", "reassessment_needed", "reassessing", "archived"}:
    ...
    if decision.variants or decision.scenarios:
        return "in_review"
    return decision_status or "draft"
...
def decision_detail(decision_id):
    with session_scope() as session:
        decision = session.get(Decision, decision_id)
        if decision is None:
            return redirect(url_for("ui.home", message="Decision not found", level="warning"))
        return render_template(
